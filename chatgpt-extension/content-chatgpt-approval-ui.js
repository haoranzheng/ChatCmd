(() => {
  ...
  function render(nextItems, soundEnabled = approvalSoundEnabled) {
    ...
    document.title = `${current.kind === 'plan' ? extTr('aiNeedsInfo', 'AI needs more information') : extTr('approvalRequired', 'Approval required')} · ChatCMD`;
      ...
      <div class="backdrop" role="alertdialog" aria-modal="true" aria-labelledby="chatcmd-approval-title" aria-describedby="chatcmd-approval-description">
        <section class="card">
          <header class="header">
            ...
            <div class="copy"><div class="eyebrow">ChatCMD · ${current.kind === 'plan' ? extTr('planningMode', 'Planning mode') : extTr('globalApproval', 'Global approval')}</div><h2 id="chatcmd-approval-title">${current.kind === 'plan' ? extTr('aiNeedsInfo', 'AI needs more information') : extTr('approvalRequired', 'Approval required')}</h2><p class="desc" id="chatcmd-approval-description">${escapeHtml(current.kind === 'plan' ? extTr('planWaiting', 'The AI is waiting for an answer in the current planning turn.') : current.kind === 'conversation' ? extTr('conversationWaiting', 'A new conversation is waiting for permission to run commands.') : extTr('activityWaiting', 'The Agent is waiting for permission to perform an action on your computer.'))}</p></div>
            <span class="count">${items.length} ${extTr('waiting', 'waiting')}</span>
          </header>
          <div class="body">
            <div class="meta"><div><strong>${escapeHtml(current.kind === 'plan' ? extTr('planningMode', 'Planning mode') : current.title || current.tool || current.taskId)}</strong><small>${extTr('task', 'Task')} #${escapeHtml(shortId(current.taskId))}</small></div><span class="timer" data-timer></span></div>
            ${current.kind === 'activity' ? `<div class="tool">${extTr('tool', 'Tool')} <code>${escapeHtml(current.tool || 'tool')}</code></div><pre>${escapeHtml(formatInput(current.input))}</pre>` : ''}
            ${current.kind === 'plan' ? `<div class="plan-question">${escapeHtml(current.question || '')}</div><div class="plan-options"><button class="plan-option" data-action="plan-option-1">1 · ${escapeHtml(current.options?.[0] || '')}</button><button class="plan-option" data-action="plan-option-2">2 · ${escapeHtml(current.options?.[1] || '')}</button></div><div class="reject"><label for="chatcmd-plan-custom">${extTr('anotherAnswer', 'Another answer')}</label><textarea id="chatcmd-plan-custom" maxlength="2000" placeholder="${extTr('anotherAnswerPlaceholder', 'Enter another suggestion for the AI…')}">${escapeHtml(reason)}</textarea></div>` : rejecting ? `<div class="reject"><label for="chatcmd-reject-reason">${extTr('rejectionReason', 'Rejection reason (optional)')}</label><textarea id="chatcmd-reject-reason" maxlength="2000" placeholder="${extTr('rejectionPlaceholder', 'Example: Do not run this command; use a read-only approach.')}">${escapeHtml(reason)}</textarea></div>` : ''}
            ${error ? `<p class="error" role="alert">${escapeHtml(error)}</p>` : ''}
          </div>
          <footer class="actions">
            ${current.kind === 'plan' ? `<button class="primary" data-action="plan-custom">${extTr('sendAnotherAnswer', 'Send another answer')}</button>` : rejecting ? `<button class="secondary" data-action="cancel-reject">${extTr('back', 'Back')}</button><button class="danger" data-action="confirm-reject">${extTr('confirmReject', 'Confirm rejection')}</button>` : `<button class="danger" data-action="reject">${extTr('reject', 'Reject')}</button>${current.kind === 'activity' ? `<button class="similar" data-action="allowSimilar">${extTr('allowSimilar', 'Allow similar')}</button>` : ''}<button class="primary" data-action="allow">${extTr('approve', 'Approve')}</button>`}
          </footer>
        </section>
      </div>
    `;
    ...
    try {
      ...
      if (current.kind === 'plan') {
        if (action === 'plan-option-1' || action === 'plan-option-2') {
          ...
          if (!reason.trim()) throw new Error(extTr('enterAnswer', 'Enter another answer before sending.'));
      ...
      if (!response?.ok) throw new Error(response?.error || extTr('sendDecisionFailed', 'Could not send the approval decision to ChatCMD.'));
      items = items.filter((item) => item.key !== current.key);
      rejecting = false;
      reason = '';
    } catch (failure) {
      error = failure instanceof Error ? failure.message : String(failure || extTr('approvalDecisionFailed', 'Could not send the approval decision.'));
    } finally {
      busy = false;
      render(items);
    }
  ...
  function updateTimer(shadow, current) {
    ...
    if (!Number.isFinite(deadline) || deadline <= 0) { target.textContent = extTr('waitingStatus', 'Waiting'); return; }
  ...
  function formatInput(value) {
    if (value == null) return extTr('noInput', 'No input data.');
    try {
